...
void InputManager::Update()
{
    ...
    //
    //    //Mouse keys
    //    current_input.m_mousekeys.x = (bool)glfwGetMouseButton(_window, GLFW_m_mouseBUTTON_LEFT);
    //    current_input.m_mousekeys.y = (bool)glfwGetMouseButton(_window, GLFW_m_mouseBUTTON_RIGHT);
    //
    //    //Mouse
    //    glfwGetCursorPos(_window, &current_input.mouse.x, &current_input.mouse.y);
}
...
{
    return m_old_input.m_keys[key] && !m_current_input.m_keys[key];
}

inline bool InputManager::KeyDown(int key)
{
    return m_current_input.m_keys[key] && !m_old_input.m_keys[key];
}

inline bool InputManager::IsDown(int key)
{
    return m_current_input.m_keys[key];
}

inline Vec2d InputManager::GetMousePosition() const
{
    return m_current_input.m_mouse;
}

inline Vec2d InputManager::GetMouseOldPosition() const
{
    return m_old_input.m_mouse;
}

Vec2d InputManager::GetMouseMovement() const
{
    return {m_current_input.m_mouse.x - m_old_input.m_mouse.x, m_current_input.m_mouse.y - m_old_input.m_mouse.y};
}
